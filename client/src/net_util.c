...
void prepare_paxos_message(struct rte_mbuf *created_pkt, uint16_t port,
  ...
  created_pkt->data_len = whole_pkt_size;
  created_pkt->pkt_len = ip_pkt_size;
  created_pkt->l2_len = sizeof(struct ether_hdr);
  created_pkt->l3_len = sizeof(struct ipv4_hdr);
  created_pkt->l4_len = l4_len;
  printf("data len %u pkt_len %u l2 %u l3 %u l4 %u\n",
    created_pkt->data_len,
    created_pkt->pkt_len,created_pkt->l2_len,
    created_pkt->l3_len,created_pkt->l4_len
