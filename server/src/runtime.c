...
app_lcore_io_rx_buffer_to_send(struct app_lcore_params_io *lp, uint32_t worker,
  ...
  //
  //         RTE_LOG(DEBUG, ROCKS, "\tI/O RX %u out (worker %u): enq success rate = %.2f\n",
  //                lcore,
  //                (unsigned)worker,
...
static inline void app_lcore_io_rx(struct app_lcore_params_io *lp,
  ...
  for (i = 0; i < lp->rx.n_nic_queues; i++) {
    ...
    // RTE_LOG(INFO, ROCKS, "Port %u Queue %u n_mbuf %u\n", port, queue, n_mbufs);

    if (unlikely(n_mbufs == 0)) {
      continue;
    }

    RTE_LOG(INFO, ROCKS, "Received %u n_mbuf\n", n_mbufs);
    ...
    //
    //             RTE_LOG(DEBUG, ROCKS, "I/O RX %u in (NIC port %u): NIC drop ratio = %.2f avg
    //             burst size = %.2f\n",
    //                    lcore,
    //                    port,
...
static inline void app_lcore_io_tx(struct app_lcore_params_io *lp,
  ...
  for (worker = 0; worker < n_workers; worker++) {
    ...
    for (i = 0; i < lp->tx.n_nic_ports; i++) {
      ...
      RTE_LOG(DEBUG, ROCKS, "worker %u n_mbuf %u dequeued %u out port %u\n", worker, n_mbufs, ret, port);
      ...
      //                 unsigned lcore = rte_lcore_id();
      //
      //                 RTE_LOG(DEBUG, ROCKS, "\t\t\tI/O TX %u out (port %u): avg burst size =
      //                 %.2f\n",
      //                        lcore,
      //                        port,
...
static inline void app_lcore_io_tx_flush(struct app_lcore_params_io *lp) {
  ...
  for (i = 0; i < lp->tx.n_nic_ports; i++) {
    ...
    RTE_LOG(DEBUG, ROCKS, "Prepare to Flush %u out Port %u\n", lp->tx.mbuf_out[port].n_mbufs, port);
    ...
    RTE_LOG(DEBUG, ROCKS, "Flush Port %u\n", port);

    n_pkts = rte_eth_tx_burst(port, 0, lp->tx.mbuf_out[port].array,
                              (uint16_t)lp->tx.mbuf_out[port].n_mbufs);
    RTE_LOG(DEBUG, ROCKS, "Flushed %u out port %u\n", n_pkts, port);
    ...
    for (j = 0; j < bsz_rd; j++) {
      ...
      ret = rte_ring_sp_enqueue_bulk(
          lp->rings_out[port], (void **)lp->mbuf_out[port].array, bsz_wr, NULL);
                  ...
                  {
                      RTE_LOG(DEBUG, ROCKS, "\t\tWorker %u out (NIC port %u): enq success rate = %.2f\n",
...
static inline void app_lcore_worker_flush(struct app_lcore_params_worker *lp) {
  ...
  for (port = 0; port < APP_MAX_NIC_PORTS; port++) {
    ...
    RTE_LOG(DEBUG, ROCKS, "Lcore %u prepare to flush %u packets\n", lp->lcore, lp->mbuf_out[port].n_mbufs);
    ret = rte_ring_sp_enqueue_bulk(lp->rings_out[port],
                                   (void **)lp->mbuf_out[port].array,
                                   lp->mbuf_out[port].n_mbufs, NULL);

    RTE_LOG(DEBUG, ROCKS, "Lcore %u flushed %u packets\n", lp->lcore, ret);
...
int app_lcore_main_loop(__attribute__((unused)) void *arg) {
  ...
  if (lp->type == e_APP_LCORE_IO) {
    RTE_LOG(DEBUG, ROCKS, "Logical core %u (I/O) main loop.\n", lcore);
  ...
  if (lp->type == e_APP_LCORE_WORKER) { // app_get_lcores_worker uses the same
    ...
    RTE_LOG(DEBUG, ROCKS, "Logical core %u (worker %u) main loop.\n", lcore,
